...
interface LogProcessor<IL extends ILog> {
        ArrayList<IL> processLogs(ArrayList<IL> logs);
        }

class LogSystem<IL extends ILog> {
    ...
    public LogSystem(ArrayList<IL> elements) {
        this.logs = elements;
    }
    ...
    void printResults() {
        ...
        LogProcessor<IL> firstLogProcessor = log->log.stream().filter(x->x.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<IL> secondLogProcessor = log->log.stream().filter(x->x.getMessage().length()<100).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<IL> thirdLogProcessor = log->log.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toCollection(ArrayList::new));
