//package APS.APSKolokvium2.lab8APS.zad3;
...
public class BinaryTreeSum {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(!treeStack.isEmpty()){
            element=treeStack.pop();
            if(element==null){
                continue;
            }
            if(element.info==baranaVrednost){
                break;
            }
            treeStack.push(element.right);
            treeStack.push(element.left);
        }
        if(element==null){
            System.out.println("Element not found!");
            return;
        }
        k=tree.sumLeftSubtree(element,element.left);
        j=tree.sumRightSubtree(element,element.right);
