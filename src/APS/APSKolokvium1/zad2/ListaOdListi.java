package APS.APSKolokvium1.zad2;
...
public class ListaOdListi {
    ...
    public static long findMagicNumber(DLL<DLL<Integer>> list) {
        ...
        for (int i = 0; i < list.length(); i++) {
            DLLNode<Integer> elementSub = sublist.element.getFirst();
            for (int j = 0; j < sublist.element.length(); j++) {
                sum += elementSub.element;
                elementSub = elementSub.succ;
            }
            total *= sum;
            sum = 0;
